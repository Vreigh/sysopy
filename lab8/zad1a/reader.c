...
void* threadJob(void* arg){
  // ustaw sposob cancelowania - tutaj wszystko domyslnie, enabled i asynchronicznie
  ...
  while(1){
    ...
    for(int i=0; i<toRead; i++){
      if(strstr(records[i].text, word) != NULL){
        printf("I, thread %ld, have succeded! The word %s has been found in record: %d!\n",
        gettid(), word, records[i].id);
